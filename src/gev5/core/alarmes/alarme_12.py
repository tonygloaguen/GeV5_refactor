...
class Alarme12(threading.Thread):
    # états publics (index principal 12, miroir 120)
    alarme_resultat = {12: 0, 120: 0}
    mesure = {12: 0}
    suiv = {12: 0}
    val_max = {12: 0}
    recal = {12: 0}
    ...
    max_pos = {12: 0}
    pdf_gen = {12: 0}
    email_send_alarm = {12: 0}
    etat_acq_modbus = {12: 0}
    etat_point_chaud = {12: 0}
    ...
    def __init__(self, multiple, seuil2, D12_ON, Mode_sans_cellules, somme_det, suiv_block):
    ...
    def run(self):
        ...
        while self.D_ON:
            try:
                self.valeur = comptage_12.Frequence_12_Thread.compteur[12]
            except Exception:
                self.valeur = 0

            if self.suiveur == 0 and self.valeur > 0:
                self.suiveur = self.valeur * self.multiple
                self.suiv[12] = self.suiveur
            ...
            if self.mesure[12] == 0 and not self.waiting_for_stable_one:
                self.last_zero_time = time.time()
                self.waiting_for_stable_one = True
            if self.mesure[12] == 1 and self.waiting_for_stable_one:
                if time.time() - self.last_zero_time >= 1.0:
                    self.val_max[12] = self.valeur
                self.waiting_for_stable_one = False
            ...
            if self.mesure[12] in [1, 2] and self.valeur > self.val_max[12]:
                self.val_max[12] = int(self.valeur)

            # Escalade N2
            if (self.valeur > self.seuil_haut
                and self.alarme_resultat[12] in (0, 1)
    ...
    def update_defauts(self):
        ...
        if self._prev_somme_def > 0 and self.somme_def == 0:
            ...
            self.etat_point_chaud[12] = 0
            if self.valeur > 0:
                self.suiveur = self.suiv[12] = self.valeur * self.multiple
                self.recal[12] = 1
                self.t_suiv = 0
    ...
    def should_reset_alarme(self):
        return (self.acq == 1 or self.etat_acq_modbus[12] == 1) and (self.alarme_resultat[12] in [1, 2])

    def reset_alarm(self):
        self.alarme_resultat[12] = 0
        self.alarme_resultat[120] = 0
        self.t_suiv = 0
        self._alarm_inhibit_until = time.time() + 3.0
        if self.mss == 1:
            self.mesure[12] = 0
            self.pdf_gen[12] = 1
            self.email_send_alarm[12] = 0

    def is_in_valid_range(self):
        try:
            v = getattr(comptage_12.Frequence_12_Thread, 'variance', {})
    ...
    def handle_suiv(self):
        if 0 not in self.liste and self.alarme_resultat[12] == 0:
            self.t_suiv += 1
    ...
    def update_suiveur(self):
        ...
        try:
            LD = getattr(comptage_12.Frequence_12_Thread, 'LD', {})
    ...
    def update_suiv_list(self):
        ...
        if self.val_max[12] < self.valeur:
            self.val_max[12] = int(self.valeur)
    ...
    def should_start_measurement(self):
        return (0 in self.liste and self._defects_ratio_ok() and self.acq == 0 and self.alarme_resultat[12] == 0)
    ...
    def start_measurement(self):
        ...
        self.val_deb_mes[12] = self.valeur
        self.suiv[12] = self.suiveur
        if self.suiveur_2 < self.suiveur:
            self.recal[12] = 1
            self.suiveur = self.suiveur_2 = self.suiv[12]
        if self.valeur > self.seuil_haut and self.alarme_resultat[12] == 0:
            self.trigger_alarm_2()
        if self.valeur > self.suiveur and self.alarme_resultat[12] == 0:
            self.trigger_alarm()

    def trigger_alarm_2(self):
        if self.mesure.get(12, 0) == 0 or (hasattr(self, 't_t_w') and time.time() < self.t_t_w):
            return
        if self.alarme_resultat.get(12, 0) < 2:
            self.alarme_resultat[12] = 2
        if self.alarme_resultat.get(120, 0) < 2:
            self.alarme_resultat[120] = 2
        self.mesure[12] = 2
        self.email_send_alarm[12] = 1

    def trigger_alarm(self):
        if self.mesure.get(12, 0) == 0 or (hasattr(self, 't_t_w') and time.time() < self.t_t_w):
            return
        if self.alarme_resultat.get(12, 0) < 1:
            self.alarme_resultat[12] = 1
        if self.alarme_resultat.get(120, 0) < 1:
            self.alarme_resultat[120] = 1
        self.mesure[12] = 2
        self.email_send_alarm[12] = 1
