...
class Frequence_7_Thread(threading.Thread):
    compteur = {7: 0, 70: 0}
    variance = {7: 0}
    LD = {7: 0}

    def __init__(self, sampling, D7_ON, base_url, sim):
    ...
    def increaserev_7(self, channel):
        self.frequence += 1
        ...
        while True:
            if self.D7_ON == 1:
                ...
                if all(alarme.pdf_gen[i] == 0 for i, alarme in enumerate([
                    ...
                    self.compteur[70] = moy

                    # Si pas de défaut => on expose la valeur, sinon on force à 0
                    if defaut_7.Defaut_7.eta_defaut[7] == 0:
                        self.compteur[7] = moy
                    else:
                        self.compteur[7] = 0
                    ...
                    self.LD[7] = stat + 8.4 * sqrt(stat)
                    
                    # variance protégée
                    if self.compteur[7] != 0:
                        self.variance[7] = self.compteur_old / self.compteur[7]
                    self.compteur_old = self.compteur[7]
    ...
    def fetch_and_process_data(self, base_url):
        ...
        # Mapping local: sur ce hub, la voie 7 correspond à la série 'compteur_3'.
        key = "compteur_3"
        series = data.get(key, [])
