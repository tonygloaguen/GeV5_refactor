...
class Frequence_9_Thread(threading.Thread):
    compteur = {9: 0, 90: 0}
    variance = {9: 0}
    LD = {9: 0}

    def __init__(self, sampling, D9_ON, base_url, sim):
    ...
    def increaserev_9(self, channel):
        self.frequence += 1
        ...
        while True:
            if self.D9_ON == 1:
                ...
                if all(alarme.pdf_gen[i] == 0 for i, alarme in enumerate([
                    ...
                    self.compteur[90] = moy

                    # Si pas de défaut => on expose la valeur, sinon on force à 0
                    if defaut_9.Defaut_9.eta_defaut[9] == 0:
                        self.compteur[9] = moy
                    else:
                        self.compteur[9] = 0
                    ...
                    self.LD[9] = stat + 8.4 * sqrt(stat)
                    
                    # variance protégée
                    if self.compteur[9] != 0:
                        self.variance[9] = self.compteur_old / self.compteur[9]
                    self.compteur_old = self.compteur[9]
    ...
    def fetch_and_process_data(self, base_url):
        ...
        # Mapping local: sur ce hub, la voie 9 correspond à la série 'compteur_1'.
        key = "compteur_1"
        series = data.get(key, [])
