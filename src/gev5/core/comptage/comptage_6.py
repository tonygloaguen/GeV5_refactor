...
class Frequence_6_Thread(threading.Thread):
    compteur = {6: 0, 60: 0}
    variance = {6: 0}
    LD = {6: 0}

    def __init__(self, sampling, D6_ON, base_url, sim):
    ...
    def increaserev_6(self, channel):
        self.frequence += 1
        ...
        while True:
            if self.D6_ON == 1:
                ...
                if all(alarme.pdf_gen[i] == 0 for i, alarme in enumerate([
                    ...
                    self.compteur[60] = moy

                    # Si pas de défaut => on expose la valeur, sinon on force à 0
                    if defaut_6.Defaut_6.eta_defaut[6] == 0:
                        self.compteur[6] = moy
                    else:
                        self.compteur[6] = 0
                    ...
                    self.LD[6] = stat + 8.4 * sqrt(stat)
                    
                    # variance protégée
                    if self.compteur[6] != 0:
                        self.variance[6] = self.compteur_old / self.compteur[6]
                    self.compteur_old = self.compteur[6]
    ...
    def fetch_and_process_data(self, base_url):
        ...
        # Mapping local: sur ce hub, la voie 6 correspond à la série 'compteur_2'.
        key = "compteur_2"
        series = data.get(key, [])
