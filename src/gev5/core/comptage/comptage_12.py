...
class Frequence_12_Thread(threading.Thread):
    compteur = {12: 0, 120: 0}
    variance = {12: 0}
    LD = {12: 0}

    def __init__(self, sampling, D12_ON, base_url, sim):
    ...
    def increaserev_12(self, channel):
        self.frequence += 1
        ...
        while True:
            if self.D12_ON == 1:
                ...
                if all(alarme.pdf_gen[i] == 0 for i, alarme in enumerate([
                    ...
                    self.compteur[120] = moy

                    # Si pas de défaut => on expose la valeur, sinon on force à 0
                    if defaut_12.Defaut_12.eta_defaut[12] == 0:
                        self.compteur[12] = moy
                    else:
                        self.compteur[12] = 0

                    # LD (bruit de fond + 8.4 * sqrt)
                    stat = sum(self.fifo_stat) / max(1, len(self.fifo_stat))
                    self.LD[12] = stat + 8.4 * sqrt(stat)
                    
                    # variance protégée
                    if self.compteur[12] != 0:
                        self.variance[12] = self.compteur_old / self.compteur[12]
                    self.compteur_old = self.compteur[12]
    ...
    def fetch_and_process_data(self, base_url):
        ...
        # Mapping local: sur ce hub, la voie 12 correspond à la série 'compteur_4'.
        key = "compteur_4"
        series = data.get(key, [])
