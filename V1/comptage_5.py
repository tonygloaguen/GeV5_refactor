...
class Frequence_5_Thread(threading.Thread):
    compteur = {5: 0, 50: 0}
    variance = {5: 0}
    LD = {5: 0}

    def __init__(self, sampling, D5_ON, base_url, sim):
    ...
    def increaserev_5(self, channel):
        self.frequence += 1
        ...
        while True:
            if self.D5_ON == 1:
                ...
                if all(alarme.pdf_gen[i] == 0 for i, alarme in enumerate([
                    ...
                    self.compteur[50] = moy

                    # Si pas de défaut => on expose la valeur, sinon on force à 0
                    if defaut_5.Defaut_5.eta_defaut[5] == 0:
                        self.compteur[5] = moy
                    else:
                        self.compteur[5] = 0
                    ...
                    self.LD[5] = stat + 8.4 * sqrt(stat)
                    
                    # variance protégée
                    if self.compteur[5] != 0:
                        self.variance[5] = self.compteur_old / self.compteur[5]
                    self.compteur_old = self.compteur[5]
    ...
    def fetch_and_process_data(self, base_url):
        ...
        # Mapping local: sur ce hub, la voie 5 correspond à la série 'compteur_1'.
        key = "compteur_1"
        series = data.get(key, [])
