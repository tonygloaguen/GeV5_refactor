...
class Frequence_8_Thread(threading.Thread):
    compteur = {8: 0, 80: 0}
    variance = {8: 0}
    LD = {8: 0}

    def __init__(self, sampling, D8_ON, base_url, sim):
    ...
    def increaserev_8(self, channel):
        self.frequence += 1
        ...
        while True:
            if self.D8_ON == 1:
                ...
                if all(alarme.pdf_gen[i] == 0 for i, alarme in enumerate([
                    ...
                    self.compteur[80] = moy

                    # Si pas de défaut => on expose la valeur, sinon on force à 0
                    if defaut_8.Defaut_8.eta_defaut[8] == 0:
                        self.compteur[8] = moy
                    else:
                        self.compteur[8] = 0
                    ...
                    self.LD[8] = stat + 8.4 * sqrt(stat)
                    
                    # variance protégée
                    if self.compteur[8] != 0:
                        self.variance[8] = self.compteur_old / self.compteur[8]
                    self.compteur_old = self.compteur[8]
    ...
    def fetch_and_process_data(self, base_url):
        ...
        # Mapping local: sur ce hub, la voie 8 correspond à la série 'compteur_4'.
        key = "compteur_4"
        series = data.get(key, [])
